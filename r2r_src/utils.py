...
def read_img_features(feature_store, test_only=False):
    ...
    if not test_only:
        features = {}
        if feature_store.endswith(".hdf5"):
            with h5py.File(feature_store, "r") as f:
                long_ids = list(f.keys())
                for long_id in long_ids:
                    features[long_id] = f[long_id][...].astype(np.float32)[1:, :768]     # CLIP feature is (36, 768)
        else:
